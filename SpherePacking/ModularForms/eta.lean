...
/--
The Dedekind eta function is defined as $$\eta(z) = q^{1/24} \prod_{n \ge 1} (1 - q^n)$$ where
$q = e^{2\pi i z}$.
-/
@[blueprint]
...
/-- The Dedekind eta function transforms as $$\eta\left(-\frac{1}{z}\right) = \sqrt{-iz} \eta(z).$$ -/
@[blueprint
  (proof := /--
  Cosider the logarithmic derivative of $\eta$, which one can easily see is equal to
  $\frac{\pi i}{12} E_2$. The result then follows from the transformation of $E_2$.
