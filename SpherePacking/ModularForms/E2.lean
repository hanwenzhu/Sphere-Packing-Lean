...
/--
$$\begin{equation}
\label{eqn:E2-transform-general}
...
@[blueprint
  (proof := /--
  Use the fact that $\mathrm{SL}_2(\mathbb{Z})$ is generated by $S$ and $T$. Then
  \ref{eqn:E2-transform-general} follows from
  \ref{eqn:E2-S-transform} and $E_2|_T = E_2$.
  -/)
  (latexEnv := "lemma")]
...
/--
This function is not modular, however it satisfies $$\begin{equation}
\label{eqn:E2-S-transform}
    z^{-2}\,E_2\left(-\frac{1}{z}\right) = E_2(z) -\frac{6i}{\pi}\, \frac{1}{z}.
\end{equation}$$
-/
@[blueprint
  (proof := /-- This is exercise 1.2.8 of [first] [course]. -/)
  (latexEnv := "lemma")]
