...
/--
Let $f(z)$ be a holomorphic function with a Fourier expansion $$\begin{equation}
    f(z) = \sum_{n \ge n_0} c_f(n) e^{\pi i n z}
\end{equation}$$ with $c_f(n_0) \ne 0$. Assume that $c_f(n)$ has a polynomial growth, i.e.
$|c_f(n)| = O(n^k)$ for some $k \in \N$. Then there exists a constant $C_f > 0$ such that
$$\begin{equation}
    \left|\frac{f(z)}{\Delta(z)}\right| \le C_f e^{-\pi (n_0 - 2) \Im z}
\end{equation}$$ for all $z$ with $\Im z > 1/2$.
-/
@[blueprint
  (proof := /--
  By the product formula \ref{eqn:disc-definition}, $$\begin{align}
  ...
  \end{equation}$$ Note that the summation in the numerator converges absolutely because of polynomial
  growth. The denominator also converges, which is simply $e^{\pi} \cdot \Delta(i/2)$.
  -/)
  (latexEnv := "lemma")]
