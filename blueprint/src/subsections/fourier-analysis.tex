...
\inputleannode{Real.fourierIntegral}

The following computational result will be of use later on.

\begin{lemma}\label{lemma:Gaussian-Fourier}\uses{Real.fourierIntegral}
  \begin{equation}
...
\inputleannode{SchwartzMap}

\inputleannode{SchwartzMap.fourierTransformCLM}
...
\begin{lemma}\label{lemma:Schwartz-summable}\uses{SchwartzMap}
...
\begin{theorem}[Poisson summation formula]\label{thm:Poisson-summation-formula}\uses{Real.fourierIntegral, SchwartzMap, lemma:Schwartz-summable, lemma:inv-power-summable, LinearMap.BilinForm.dualSubmodule}
